...
export class NebulaProvider implements Provider {
    ...
    async search(query: string, options: SearchOptions): Promise<unknown[]> {
        ...
        // Check for SDK format (MemoryResponse) containing semantic/episodic/procedural memory
        if ((results as any).sources || (results as any).semantics || (results as any).episodes || (results as any).procedures) {
            return [results]
        }
